    ...
    ILogger<SigningService> logger)
{
    ...
    {
        var (success, output, diagnostics) = await ExecuteGnuCommand(
    ...
    {
        var (success, signature, diagnostics) = await ExecuteGnuCommand(
    ...
    {
        var (success, fingerprint, diagnostics) = await ExecuteGnuCommand("gpg --list-secret-keys --with-colons --keyid-format=long | awk -F: '$1==\"fpr\" {printf $10; exit}'");
