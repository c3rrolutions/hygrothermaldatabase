...
import { AllLifeCycleDataDocument } from "../../../queries/data.generated";
import {
	LifeCycleData,
	Scalars,
	...
	const onFinish = ({
		...
		const filter = async () => {
			try {
				...
				// TODO Casting to `LifeCycleData` is wrong and error prone!
				setData(
					(data?.allLifeCycleData?.edges?.map((x) => x.node) ||
						[]) as LifeCycleData[],
				);
			} catch (error) {
				// TODO Handle properly.
